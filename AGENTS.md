...
- **release-requires-remote-migrations**：
  - 约束/适用范围：涉及后端或数据库变更的发布必须执行远程 migration，并对关键 API 做线上冒烟验证。
  - 示例：数据库字段变更发布前执行 migration，并验证关键接口返回。
  - 反例：变更数据库后直接发布，不跑 migration 或不做线上验证。
  - 执行方式：发布流程中包含 migration + 线上关键 API 冒烟步骤。
  - 维护责任人：当次交付 owner。
- **release-must-be-closed-loop**：
  - 约束/适用范围：任何“发布/上线”必须形成闭环：migrations apply → deploy → 线上冒烟验证；缺一不可。
  - 示例：完成部署后立刻执行线上冒烟并记录结果。
  - 反例：仅部署不验证，或仅验证未部署。
  - 执行方式：发布步骤按闭环顺序执行并记录。
  - 维护责任人：当次交付 owner。
